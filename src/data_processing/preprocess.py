...
def map_binary_features(df: pd.DataFrame, binary_cols_map: dict) -> pd.DataFrame:
    ...
    for col, mapping in binary_cols_map.items():
        if col in df.columns:
            ...
            if nan_count > 0:
                logger.warning(f"{nan_count} valeurs dans '{col}' n'ont pas pu être mappées et sont devenues NaN.")
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'a_quitte_l_entreprise' in df.columns:
        df[config.TARGET_VARIABLE] = df['a_quitte_l_entreprise'].map({'Oui': 1, 'Non': 0})
    ...
    cols_to_drop = ['id_employee', 'nombre_heures_travailless', 'eval_number', 'nombre_employee_sous_responsabilite', 'code_sondage', 'ayant_enfants', 'a_quitte_l_entreprise', 'annee_experience_totale', 'niveau_hierarchique_poste', 'annees_dans_le_poste_actuel', 'annes_sous_responsable_actuel']
    ...
    if col_aug in df.columns:
        ...
        logger.warning(f"La colonne '{col_aug}' n'a pas été trouvée pour la conversion.")
...
def build_preprocessor(numerical_cols: list, onehot_cols: list, ordinal_cols: list, ordinal_categories: dict) -> ColumnTransformer:
    ...
    if ordinal_cols:
        for col in ordinal_cols:
            ...
            ordinal_transformer_col = Pipeline(steps=[
                ('imputer', SimpleImputer(strategy='most_frequent')),
                ('ordinal', OrdinalEncoder(categories=categories_for_col, handle_unknown='use_encoded_value', unknown_value=-1))
            ])
...
def run_preprocessing_pipeline(df: pd.DataFrame,
    ...
    numerical_to_scale = [col for col in potential_numerical_cols if col not in ordinal_to_encode]

    onehot_to_encode = [col for col in X.select_dtypes(include=['object', 'category']).columns.tolist() if col not in ordinal_to_encode]
    ...
    if fit:
        processor_instance = build_preprocessor(numerical_to_scale, onehot_to_encode, ordinal_to_encode, ordinal_cols_categories or {})
        ...
        try:
            ...
            logger.warning(f"Impossible d'obtenir les noms de features: {e}. Noms génériques utilisés.")
            feature_names = [f"feature_{i}" for i in range(X_processed.shape[1])]
        logger.info("Transformation terminée.")
        return pd.DataFrame(X_processed, columns=feature_names, index=X.index), y, processor_instance
    else:
        if preprocessor:
...
if __name__ == "__main__":
    ...
    if df_raw is not None:
        ...
        binary_features_mapping = {
            ...
            'heure_supplementaires': {'Non': 0, 'Oui': 1} # <-- AJOUTÉ ICI ! Adaptez 'Non'/'Oui' si nécessaire.
        }
        ...
        try:
            X_p, y_p, proc = run_preprocessing_pipeline(df_raw,
            ...
            if genre_col: print(f"\nColonne 'genre' traitée (scalée) : {genre_col[0]}")
            if hs_col: print(f"Colonne 'heure_supplementaires' traitée (scalée) : {hs_col[0]}")
