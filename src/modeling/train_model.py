...
from src.data_processing.preprocess import clean_data, map_binary_features, create_features, build_preprocessor
...
def train_and_evaluate_pipeline():
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=0.20, random_state=42, stratify=y # Stratify est important pour les classes déséquilibrées
    )
    ...
    potential_numerical_cols = X_train.select_dtypes(include=[np.number]).columns.tolist()
    ...
    onehot_to_encode = X_train.select_dtypes(include=['object', 'category']).columns.tolist()
