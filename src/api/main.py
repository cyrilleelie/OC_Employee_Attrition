...
async def predict_single(employee_data: EmployeeInput):
    if load_prediction_pipeline() is None: # Vérifie si la pipeline est chargée (elle devrait l'être par lifespan)
        raise HTTPException(
    ...
    try:
        ...
        logger.info(f"Prédiction pour 1 employé : {employee_data.model_dump(exclude_none=True)}")
        ...
        if not results:
             raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="La prédiction a échoué sans message.")
        return results[0]
