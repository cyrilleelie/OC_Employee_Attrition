...
print(f"Allowed genre keys from config: {list(config.BINARY_FEATURES_MAPPING['genre'].keys())}")
print(f"Allowed heure_supplementaires keys from config: {list(config.BINARY_FEATURES_MAPPING['heure_supplementaires'].keys())}")
print(f"Allowed frequence_deplacement values from config: {config.ORDINAL_FEATURES_CATEGORIES['frequence_deplacement']}")
print(f"Allowed statut_marital values from config: {config.LIMITED_VALUES_FEATURES['statut_marital']}")
print(f"Allowed departement values from config: {config.LIMITED_VALUES_FEATURES['departement']}")
print(f"Allowed poste values from config: {config.LIMITED_VALUES_FEATURES['poste']}")
print(f"Allowed domaine_etude values from config: {config.LIMITED_VALUES_FEATURES['domaine_etude']}")
print("-" * 20)
...
class EmployeeInput(BaseModel):
    ...
    genre: str = Field(
        ..., 
        json_schema_extra={"example": "F"}, # Modifié pour correspondre à la description
    ...
    departement: str = Field(
        ...,
        json_schema_extra={"example": "Commercial"}, # 'Ventes' au lieu de 'Commercial' pour un département
    ...
    nombre_experiences_precedentes: int = Field(
        ...,
        json_schema_extra={"example": 3}, # J'ai mis 3 au lieu de 8 pour varier de l'exemple global
    ...
    augementation_salaire_precedente: str = Field( # Note: "augementation" a une petite coquille, devrait être "augmentation"
        ...,
    ...
    niveau_education: int = Field( # Si c'est une échelle numérique (ex: 1=Bac, 2=Bac+2...)
        ..., 
        json_schema_extra={"example": 3}, 
    ...
    def validate_genre(cls, value: str) -> str:
        ...
        if value not in allowed_values:
            raise ValueError(f"Valeur pour 'genre' non valide. Doit être l'une de : {list(allowed_values)}")
        return value
    ...
    def validate_heure_supplementaires(cls, value: str) -> str:
        ...
        if value not in allowed_values:
            raise ValueError(f"Valeur pour 'heure_supplementaires' non valide. Doit être l'une de : {list(allowed_values)}")
        return value
    ...
    def validate_frequence_deplacement(cls, value: str) -> str:
        ...
        if value not in allowed_values:
            raise ValueError(f"Valeur pour 'frequence_deplacement' non valide. Doit être l'une de : {list(allowed_values)}")
        return value
    ...
    def validate_statut_marital(cls, value: str) -> str:
        ...
        if value not in allowed_values:
            raise ValueError(f"Valeur pour 'statut_marital' non valide. Doit être l'une de : {list(allowed_values)}")
        return value
    ...
    def validate_departement(cls, value: str) -> str:
        ...
        if value not in allowed_values:
            raise ValueError(f"Valeur pour 'departement' non valide. Doit être l'une de : {list(allowed_values)}")
        return value
    ...
    def validate_poste(cls, value: str) -> str:
        ...
        if value not in allowed_values:
            raise ValueError(f"Valeur pour 'poste' non valide. Doit être l'une de : {list(allowed_values)}")
        return value
    ...
    def validate_domaine_etude(cls, value: str) -> str:
        ...
        if value not in allowed_values:
            raise ValueError(f"Valeur pour 'domaine_etude' non valide. Doit être l'une de : {list(allowed_values)}")
        return value
...
class PredictionOutput(BaseModel):
    ...
    """
    id_employe: Union[str, int] # L'ID peut être un str (ex: index du DataFrame de prédiction) ou un int
    probabilite_depart: float = Field(
        ..., # Requis
        ge=0.0, le=1.0, # Ajout de contraintes de validation (>=0 et <=1)
